...
void main()
{
    ...
    auto comp = zcompressedOutput(appender(&storage));

    foreach (i; 0 .. 16)
        comp.put("オレンジジュースに緑の食紅をまぜるとキウイ味になるよ\n");

    comp.put([ "ほ", "ん", "と", "だ", "よ" ]);
    comp.put('\n');
    comp.close();

    File("test01-out.deflate", "wb").rawWrite(storage);
}
...
/**
 * Returns a filtering output range that compresses (deflates) put data with
 * zlib and sends the compressed data to sink.
 *
 * Params:
 *   sink  = output stream which accepts compressed data
 *   level = zlib compression _level, which must be between 1 and 9
 */
auto zcompressedOutput(R)(R sink)
    if (isOutputRange!(R, const(ubyte)[]))
{
    return ZCompressedOutput!(R)(sink, Z_DEFAULT_COMPRESSION);
}

/// ditto
auto zcompressedOutput(R)(R sink, int level)
    if (isOutputRange!(R, const(ubyte)[]))
{
    enforce( 1 <= level && level <= 9,
        "Specified an invalid compression level for zlib compression" );
    return ZCompressedOutput!(R)(sink, level);
}

/// ditto
struct ZCompressedOutput(R)
{
