 ...
 *
 */
auto compressedOutput(CompressionMethod method = CompressionMethod.zlib, Sink)
        (Sink sink)
    if (isOutputRange!(Sink, ubyte[]))
{
    return CompressedOutput!(method, Sink)(sink);
}
...
auto compressedOutput(CompressionMethod method = CompressionMethod.zlib, Sink)
    ...
    if (isOutputRange!(Sink, ubyte[]))
{
    return CompressedOutput!(method, Sink)(sink, level);
}

/// ditto
struct CompressedOutput(CompressionMethod compressionMethod, R)
{
...
private:
    ...
    bool deflateAndWrite(int mode)
    {
        ...
        if (auto size = buffer.length - zstream.avail_out)
            context_.sink.put(buffer[0 .. size]);
